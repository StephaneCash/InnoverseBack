        ...
        if (typeof (req.body.montant) === "object") {
                ...
                return compteModel.findOne(
                    ...
                    async (err, docs) => {
                        const response = docs.devises.find((devise) =>
                            devise._id.equals(req.body.idDevise)
                        );
                        if (response) {
                            if (response.montant > parseInt(req.body.montant[i])) {
                                response.montant = response.montant - parseInt(req.body.montant[i]);
                            ...
                            if (!err) {
                                return compteModel.findOne(
                                    ...
                                    (err, docs) => {
                                        ...
                                        if (repComment) {
                                            repComment.montant = repComment.montant + parseInt(req.body.montant[i]);
                                        } else {
            ...
            return compteModel.findOne(
                ...
                async (err, docs) => {
                    ...
                    if (response) {
                        if (response.montant > parseInt(req.body.montant)) {
                            response.montant = response.montant - parseInt(req.body.montant);
                            await transactionModel.create({
                                userId: req.body.userId,
                                compteId: req.body.compteId,
                                motif: req.body.motif,
                        ...
                        if (!err) {
                            return compteModel.findOne(
                                ...
                                (err, docs) => {
                                    ...
                                    if (repComment) {
                                        repComment.montant = repComment.montant + parseInt(req.body.montant);
                                    } else {
